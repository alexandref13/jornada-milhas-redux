...
export default function Home({}: HomeProps) {
  ...
  const [pessoas, setPessoas] = useState<Filtros['pessoas']>(
    valoresPadrao.pessoas,
  );
  const [origem, setOrigem] = useState<Filtros['origem']>(valoresPadrao.origem);
  const [destino, setDestino] = useState<Filtros['destino']>(
    valoresPadrao.destino,
  );
  const [dataIda, setDataIda] = useState<Filtros['dataIda']>(
    valoresPadrao.dataIda,
  );
  const [dataVolta, setDataVolta] = useState<Filtros['dataVolta']>(
    valoresPadrao.dataVolta,
  );
  const [buscando, setBuscando] = useState<boolean>(false);
  const [filtrarPorUsuario, setFiltrarPorUsuario] = useState<
  Filtros['filtrarPorUsuario']
  ...
  const handleFiltrarPorUsuario =
    (novoFiltroPorUsuario: Filtros['filtrarPorUsuario']) => () => {
      ...
      const deveFiltrarPorUsuario =
        mostrarTodasAsViagens || novaOrigem !== origem;
      setFiltrarPorUsuario(
        deveFiltrarPorUsuario ? novoFiltroPorUsuario : 'todas',
      );
  ...
  const handleBuscar = async () => {
    ...
    if (filtrosVazios) novasViagens = todasAsViagens.current;
      ...
      novasViagens = await filtrarViagens(
        todasAsViagens.current,
        filtros,
        cidade,
        estado,
      );
    setViagens(novasViagens);
    if (!novasViagens.length)
      return criarMensagem.erro('nenhuma viagem encontrada');
    if (!filtrosVazios)
      criarMensagem.sucesso(`${novasViagens.length} viagens encontradas`);
    setBuscando(false);
  };
          ...
          <Card style={styles.passagensContent}>
            ...
            <View style={styles.tipoContainer}>
              <Button
                ...
                style={[
                  styles.tipoIda,
                  tipo === TipoViagem.idaEVolta && styles.tipoSelecionado,
                ]}
              ...
              <Button
                ...
                style={[
                  ...
                  tipo === TipoViagem.ida && styles.tipoSelecionado,
                ]}
            ...
            <View style={styles.origemContainer}>
              <StringPicker
                ...
                style={[
                  styles.origem,
                  !mostrarTodasAsViagens && styles.inputDisabled,
                ]}
                options={origens}
              />
              <TouchableRipple
                style={styles.trocar}
                onPress={trocarOrigemDestino}
              >
